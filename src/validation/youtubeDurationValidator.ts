...
export interface YoutubeDurationConfig {
  apiKey: string;
...
export class YoutubeDurationValidator {
  constructor(private readonly config: YoutubeDurationConfig) {}

  public async check(url: string): Promise<DurationCheckResult> {
    if (!isYoutubeUrl(url)) {
      return { allowed: true };
    }
    ...
    const item = (data as { items?: { contentDetails?: { duration?: string } }[] }).items?.[0];
