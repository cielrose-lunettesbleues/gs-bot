...
class AdminService {
    ...
    async handleBoolFlag(context, key, label, rest) {
        ...
        await context.reply(`@${context.user.username} ${label} ${val === "on" ? "activé" : "désactivé"}.`);
        this.deps.logger.info({ key, value: val === "on" }, `Admin: ${label} changed`);
    }
    async handleCooldown(context, rest) {
        const val = rest[0]?.toLowerCase();
        if (val === "on" || val === "off") {
            this.deps.runtimeConfig.cooldown.enabled = val === "on";
            await context.reply(`@${context.user.username} Cooldown ${val === "on" ? "activé" : "désactivé"}.`);
        }
        else {
    ...
    async handleBlock(context, rest) {
    ...
    async handleUnblock(context, rest) {
        const target = rest[0]?.toLowerCase().replace(/^@/, "");
        if (!target) {
    ...
    async handleBlocklist(context) {
        const list = this.deps.blacklistService.list();
        if (list.length === 0) {
            await context.reply(`@${context.user.username} Liste noire vide.`);
        }
        else {
            await context.reply(`@${context.user.username} Liste noire (${list.length}): ${list.slice(0, 10).join(", ")}${list.length > 10 ? "…" : ""}`);
        }
    }
    async handleApprove(context, rest) {
    ...
    async handleHistory(context, rest) {
