...
function createGreenScreenCommand(deps, commandName) {
    return {
        ...
        async execute(context, args) {
            ...
            const cooldownDecision = deps.cooldownService.checkAndConsume(deps.config.cooldown, context.user.username);
            ...
            switch (result.status) {
                case "playing":
                    if (feedback) {
                        await context.reply(`@${context.user.username} En cours (${deps.config.playback.durationSeconds}s).`);
                    }
                    break;
                case "queued":
